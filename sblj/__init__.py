...
class LONoiseLJ(StarburstLJ):
    ...
    def setLOFreq(self, freq):
        self.errorCheck()
        try:
            self.lofreq = LONoiseLJ.ljLODict[freq](self)
        except KeyError:
...
class AntennaLJ(StarburstLJ):
    ...
    pow_offset = 9.4 # added to power detector readout to account for coupler
    ...
    def __init__(self, identifier="ANY", connectionType="ETHERNET", 
        ...
        self.noiseRFswitch = {'H':-1,'V':-1}
        # -1 means status unknown
    ...
    def __getVNoiseSel(self):
        return self.noiseRFswitch['H']
    def __getHNoiseSel(self):
        return self.noiseRFswitch['V']
    ...
    def selectNoiseSource(self, list=["H","V"]):
        ...
        if "H" in list:
            ...
            self.noiseRFswitch['H'] = 1
        if "V" in list:
            ljm.eWriteName(self.handle, "EIO2", 1)
            self.noiseRFswitch['V'] = 1
